...
class MasterControllerMonir extends Controller
{
    ...
    public function profileIndex_old(Request $request)
    {
        

        $first_date_of_year = '2023-01-01';
        $leaveInfo = LeaveType::valid()->project()->where('leave_short_type','AL')->get()->toArray();
        $authorizedLive = LeaveSetup::valid()->project()->where('leave_short_type','AL')->where('leave_status', 1)->get();
        // 200645
        // 209927
        $emplyees = Employee::valid()->project()->whereIn('employee_sbu', [24])->where('employee_type', 1)->where('employee_status', 1)
        ->whereIn('employee_id_no', [100480])
        ->get();
        $annualLive = [];
        foreach ($emplyees as $key => $employee){
            ...
            foreach ($leaveInfo as $key => $value) {
                ...
                // echo "<pre>";
                //     print_r([$aviledLives,$authorizedLives,$thisYersBalance,$thisYesrsearned_day]);// 205711
                //     exit();
        ...
        $pabxAndEmailData = Employee::valid()->project()
            ...
            ->get();
        $data['emailListData'] = collect($pabxAndEmailData)->where('official_email_id', '!=', '')->where('official_email_id', '!=', 'n/a')->toArray();
        ...
        $data['pabxnumber'] = collect($pabxAndEmailData)->where('desk_phone_no', '!=', '')->toArray();  
        ...
        $attendanceTime = AttendanceSetup::valid()->project()
            ...
            ->get();
        $attendances =  $this->attendances_finds($start, $end, $data['employee_data']['employee_joining_date'], $attendance_data, $attendanceTime);
        ...
        foreach ($data['months_array'] as $month_key => $value) {
            if ($request['from_date'] && $request['to_date']) {
                ...
                if (in_array($month_key, $requestMonths)) {
                    ...
                    // $attendances =[];
                    $attendances = $this->attendances_finds($last_day_of_month, $first_date_of_month, $data['employee_data']['employee_joining_date'], $attendance_data, $attendanceTime);
    ...
    public function profileIndex(Request $request)
    {
        ...
        $data['user'] = $employee = Auth::guard('user')->user();
        // UsersPersonModel::where('id', $id)->first();
        $data['employee_data'] = Employee::valid()->project()
            ->select(
                'employees.*',
                'employee_personal_infos.*',
        ...
        $leaveNo = LeaveApplication::valid()->project()
            ...
            ->where('leave_applications.leave_apply_status', '=', 2)
            ->where('leave_to_date', '>=', $first_date_of_year)
            ->where('leave_applications.employee_id', '=', $employee['employee_id'])
            ->groupBy('leave_applications.leave_type')
            ->get()->toArray();

        $leave_consumed = array();
        $leave_available = array();
        $earnedLeave = DB::table('earned_leave')->where('employee_id',$employee['employee_id'])->where('leave_status',1)->where('date','<',$first_date_of_year)->get();
        ...
        $authorizedLive = LeaveSetup::valid()->project()->where('leave_status', 1)->get();
        $availedLive = LeaveApplication::valid()->project()->where('employee_id', $employee['employee_id'])->where('leave_apply_status', 2)
        ->where('leave_from_date','>=',$first_date_of_year)->where('leave_to_date','<=',date("Y-m-d"))
        ->get();
        $earnedLeave = DB::table('earned_leave')->where('employee_id', $employee['employee_id'])->where('leave_status', 1)->where('date', '<', $first_date_of_year)->get();
        foreach ($leaveInfo as $key => $value) {
    ...
    function attendances_finds($start, $end, $joining_date, $attendance_data, $attendanceTime)
    {
        $start_date = $end;
        $end_date = $start;
        ...
        foreach ($period_main as $date) {
            ...
            //     echo "<pre>";
            // print_r($attendance_time['office_start_time']);
            // exit();
            $office_start_time = isset($attendance_time['office_start_time']) ? $attendance_time['office_start_time'] : '00:00:00';
            $office_end_time = isset($attendance_time['office_end_time']) ? $attendance_time['office_end_time'] : '00:00:00';
    ...
    public function get_service_list_info($id)
    {
        ...
        $general_stationery_list = DB::connection('mysql2')->table('hr_stationary_summary')
            ->where('employee_id', $id)
            ->where('valid', 1)
            ->get();
        $data['general_stationery_list'] = $general_stationery_list;
        ...
        foreach ($general_stationery_list as $key => $value) {
            $serviceList[] = [
                'Type' => 'General Stationery',
                'type_id' => 7,
                'date' => $value->request_date,
                'status' => $value->status,
                'purpose' => $value->stationary_remarks,
                'id' => $value->id,
            ];
        }
    ...
    public function myProfileComponent(){
        if (!empty(Auth::guard('user')->user()->id)) {
            $id = Auth::guard('user')->user()->id;
        } else {
            return redirect('/');
       ...
       $data['others_contact_details'] = EmployeeOthersContact::valid()->project()
        ...
        return view('layouts.myProfileComponent', $data);
    }
    
    public function employeeDirectoryComponent(){
        if (!empty(Auth::guard('user')->user()->id)) {
            $id = Auth::guard('user')->user()->id;
        } else {
            return redirect('/');
        ...
        return view('layouts.employeeDirectoryComponent',$data);
    }
    
    public function fileManagerComponent(){
        ...
        $data['all_folder_info'] = $all_folder_info_data->paginate();
        /* Worked on 20/1/2021 End */
        return view('layouts.fileManagerComponent',$data);
    }
    ...
    public function payrollComponent(){
        ...
        $attendanceTime = AttendanceSetup::valid()->project()
            ...
            ->get();
        $data['employee_data'] = Employee::valid()->project()
            ->where('employees.id', Auth::guard('user')->user()->employee_id)
            ->first();
        ...
        $data['pay_days'] = $data['present_day_count'] + $data['late_day_count'] + $data['leave_count'] + $data['holiday_count'];
              ...
              ->where('provident_funds.employee_id', Auth::guard('user')->user()->employee_id)
              ->where('provident_funds.pf_status', 1)
              ->orderBy('provident_funds.id', 'desc');
          $provident_fund_info_data = $provident_fund_info;
          $data['provident_fund_info'] = $provident_fund_info = $provident_fund_info_data->get();
          ...
          $employee_loan_info = LoanAdvance::valid()->project()
              ...
              ->where('employee_loans.employee_id', Auth::guard('user')->user()->employee_id)
              ->where('employee_loans.loan_status', 1)
              ->groupBy('loan_adv_id')
              ->orderBy('employee_loans.id', 'desc');
          $employee_loan_info_data = $employee_loan_info;
        ...
        return view('layouts.payrollComponent',$data);
    }
    
    public function leaveApplyComponent(){
        
        return view('layouts.leaveApplyComponent');
    }
