...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','AttendanceIssues')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =AttendanceIssue::valid()->project()
  // ->join('employees', 'attendance_issues.id', '=', 'employees.employee_department')
  // ->leftJoin('employees as emp2', 'emp2.id', '=', 'attendance_issues.department_head')
  ->select(
    'attendance_issues.*'
  )
  ->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('attendance_issues.attendance_issue','LIKE','%'.$search_key.'%');
    });
    return $query;

  })->where('attendance_issues.project_id',$project_id)->orderBy($sort,$order);
  // ->paginate($paginate_num);
  $sortData=$paginate_data;
  $sortGetData=$sortData->get();
  $data['total_data']=count($sortGetData);
  $data['inactive_data']=count(collect($sortGetData)->whereIn('attendance_issue_status',0)->toArray());
  $data['active_data']=count(collect($sortGetData)->where('attendance_issue_status',1)->toArray());
...
public function store(Request $request)
{
  ...
  $validate=[
    'attendance_issue'=>'required'
  ];

  $request->validate($validate);
  $data=$request->only('attendance_issue','attendance_issue_status');

  if(!empty($request->id))
  {
    $update_data=AttendanceIssue::valid()->project()->findOrFail($request->id);
    ...
    $save_data=AttendanceIssue::create($data);
...
{
  $edit_data=AttendanceIssue::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=AttendanceIssue::valid()->project()->findOrFail($id);
  ...
  public function findDepartmentMaxCode(){
    $last_entry_data=AttendanceIssue::latest()->first();
