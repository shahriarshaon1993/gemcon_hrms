...
class AttendanceScheduleProcessController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $company_sbu_data = CompanySbu::valid()->project()->orderBy('sbu_name', 'ASC')->get();
         ...
         try {
                ...
                foreach ($period_main as $key => $value1) {
                    ...
                    // if(!empty($attendance_setups_data)){
                        ...
                        if(count($collect_unit_permission) > 0 && in_array($value->employee_unit, $collect_unit_permission)){

                            $holiday_permission = collect($holiday_permission)->where('unit_permission', $value->employee_unit);
                        }
                        if(count($collect_sub_unit_permission) > 0 && in_array($value->employee_sub_unit, $collect_sub_unit_permission)){

                            $holiday_permission = collect($holiday_permission)->where('sub_unit_permission', $value->employee_sub_unit);
                        }
                        if(count($collect_department_permission) > 0 && in_array($value->employee_department, $collect_department_permission)){

                            $holiday_permission = collect($holiday_permission)->where('department_permission', $value->employee_department);
                        }
                        if(count($collect_section_permission) > 0 && in_array($value->employee_section, $collect_section_permission)){

                            $holiday_permission = collect($holiday_permission)->where('section_permission', $value->employee_section);
                        }
                        if(count($collect_sub_section_permission) > 0 && in_array($value->employee_sub_section, $collect_sub_section_permission)){

                            $holiday_permission = collect($holiday_permission)->where('sub_section_permission', $value->employee_sub_section);
                        }
                        if(count($collect_work_location_permission) > 0 && in_array($value->employee_work_location, $collect_work_location_permission)){

                            $holiday_permission = collect($holiday_permission)->where('work_location_permission', $value->employee_work_location);
                        }
                        if(count($collect_employee_id_permission) > 0 && in_array($value->id, $collect_employee_id_permission)){
                    ...
                    // dd($collect_work_location_permission, $holiday_permission, $holiday_find, $attendance_setups_data);
                    ...
                    // dd($check_holiday_permission, $holiday_find, $holiday_permission);
