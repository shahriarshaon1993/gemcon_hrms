...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','AttendanceSetup')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =AttendanceSetup::valid()->project()
    ->leftJoin('employees', 'employees.id', '=', 'attendance_setups.employee_id')
    ->leftJoin('attendance_machines', 'attendance_machines.id', '=', 'attendance_setups.attendance_machine_no')
    ->leftJoin('office_time_setups', 'office_time_setups.id', '=', 'attendance_setups.attendance_office_time')
    ->select('attendance_setups.*','employees.employee_fullname','employees.employee_id_no','attendance_machines.attendance_machine_name','office_time_setups.office_start_time','office_time_setups.office_end_time')
    ->when($search_key, function($query, $search_key){
      $query->where(function($query2)use($search_key){
        $query2->where('attendance_setups.attendance_type','LIKE','%'.$search_key.'%');
        $query2->orWhere('employees.employee_id_no','LIKE','%'.$search_key.'%');
      });
      return $query;

    })->where('attendance_setups.project_id',$project_id)->whereIn('attendance_setups.employee_id',$employee_id)->orderBy($sort,$order);
    // ->paginate($paginate_num);
    // ->paginate($paginate_num);
    $sortData=$paginate_data;
    $sortGetData=$sortData->get();
    $data['total_data']=count($sortGetData);
    $data['inactive_data']=count(collect($sortGetData)->whereIn('attendance_setup_status',0)->toArray());
...
public function store(Request $request)
{
  ...
  $validate=[
    ...
    'attendance_category'=>'required',
    'attendance_machine_no'=>'required',
    'attendance_office_time'=>'required',
    'start_date'=>'required',
    'end_date'=>'required'
  ];

  $request->validate($validate);
  $data=$request->only('employee_id','attendance_type','attendance_category','attendance_machine_no','attendance_office_time','attendance_setup_status','start_date','end_date');

  if(!empty($request->id))
  {
    $update_data=AttendanceSetup::valid()->project()->findOrFail($request->id);
...
public function edit($id)
{
   ...
   $employee_ids=$employee_list->Employee_id();
   $employee_id=$employee_ids['employee_id'];
  $data=AttendanceSetup::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=AttendanceSetup::valid()->project()->findOrFail($id);
    ...
    $data['paginate_data'] =DB::table('attendance_log')
      ...
      ->when($search_key, function($query, $search_key){
        $query->where(function($query2)use($search_key){
          ...
          $query2->orWhere('attendance_log.employee_id','LIKE','%'.$search_key.'%');
        });
        return $query;
