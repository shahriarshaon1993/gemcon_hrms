...
class BankNBranchController extends Controller
{
/**
...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','BankNBranch')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =BankNBranch::valid()->project()->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('bank_name','LIKE','%'.$search_key.'%');
      $query2->orWhere('bank_code','LIKE','%'.$search_key.'%');
    });
    return $query;
  ...
  $data['inactive_data']=count(collect($sortGetData)->whereIn('bank_status',"!=",1)->toArray());
  $data['active_data']=count(collect($sortGetData)->where('bank_status',1)->toArray());
...
public function store(Request $request)
{
  ...
  $data=$request->only('bank_name','bank_status','branch_name','bank_address','priority');

  if(!empty($request->id))
  {
    $update_data=BankNBranch::valid()->project()->findOrFail($request->id);
    ...
    $data['bank_code'] = $this->findMaxCode();
    ...
    $data['bank_status']=1; 
    $save_data=BankNBranch::create($data);
...
{
  $edit_data=BankNBranch::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=BankNBranch::valid()->project()->findOrFail($id);
  ...
  public function findMaxCode(){
    $last_entry_data=BankNBranch::max('bank_code');
  ...
  public function findPriority(){
    $last_entry_data=BankNBranch::max('priority');
  ...
  {
      $data=array();
      $datas=BankNBranch::valid()->project()->orderBy('bank_name','asc')->get();
      array_push($data,['id'=>'','text'=>'Deselect']);
        foreach ($datas as $value) {
          array_push($data,['id'=>$value['id'],'text'=>$value['bank_name'],]);
      }
      $result = $data;
