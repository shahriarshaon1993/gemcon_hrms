...
class EducationalQualificationListController extends Controller
{
/**
...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','EducationalQualificationList')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =EducationalQualificationList::valid()->project()->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('degree_name','LIKE','%'.$search_key.'%');
      $query2->orWhere('degree_code','LIKE','%'.$search_key.'%');
    });
    return $query;
  ...
  $data['inactive_data']=count(collect($sortGetData)->whereIn('degree_status',"!=",1)->toArray());
  $data['active_data']=count(collect($sortGetData)->where('degree_status',1)->toArray());
...
public function store(Request $request)
{
  ...
  $data=$request->only('degree_name','degree_status','priority');

  if(!empty($request->id))
  {
    $update_data=EducationalQualificationList::valid()->project()->findOrFail($request->id);
    ...
    $data['degree_code'] = $this->findMaxCode();
    ...
    $data['degree_status']=1; 
    $save_data=EducationalQualificationList::create($data);
...
{
  $edit_data=EducationalQualificationList::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=EducationalQualificationList::valid()->project()->findOrFail($id);
  ...
  public function findMaxCode(){
    $last_entry_data=EducationalQualificationList::max('degree_code');
  ...
  public function findPriority(){
    $last_entry_data=EducationalQualificationList::max('priority');
  ...
  public function sectionSelect2()
  {
      $data=array();
      $datas=EducationalQualificationList::valid()->project()->orderBy('degree_name','asc')->get();
      array_push($data,['id'=>'','text'=>'Deselect']);
        foreach ($datas as $value) {
          array_push($data,['id'=>$value['id'],'text'=>$value['degree_name'],]);
      }
      $result = $data;
