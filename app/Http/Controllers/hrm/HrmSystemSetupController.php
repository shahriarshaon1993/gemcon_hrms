...
use Auth;
use Session;
use App\Model\HrmSystemSetup;
use Cache;
use permission;
// use App\Model\UserRoleAccess;

class HrmSystemSetupController extends Controller
{
/**
...
public function index(Request $request){
  ...
  $data['paginate_data'] =HrmSystemSetup::valid()->project()->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('hss_fiscal_year','LIKE','%'.$search_key.'%');
    });
    return $query;
...
public function store(Request $request)
{
  ...
  $validate=[
    'hss_fiscal_year'=>'required',
  ];

  $request->validate($validate);
  $data=$request->only('department_name','department_status','department_head');

  if(!empty($request->id))
  {
    $update_data=HrmSystemSetup::valid()->project()->findOrFail($request->id);
    ...
    $save_data=HrmSystemSetup::create($data);
...
{
  $edit_data=HrmSystemSetup::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=HrmSystemSetup::valid()->project()->findOrFail($id);
  ...
  public function findDepartmentMaxCode(){
    $last_entry_data=HrmSystemSetup::latest()->first();
