...
class NoteIssueController extends Controller
{
/**
...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','NoteIssues')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data  = NoteIssue::valid()->project()
  // ->join('employees', 'note_issues.id', '=', 'employees.employee_department')
  // ->leftJoin('employees as emp2', 'emp2.id', '=', 'note_issues.department_head')
  ->select(
    'note_issues.*'
  )
  ->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('note_issues.note_issue','LIKE','%'.$search_key.'%');
    });
    return $query;

  })->where('note_issues.project_id',$project_id)->orderBy($sort,$order);
  // ->paginate($paginate_num);
  $sortData=$paginate_data;
  $sortGetData=$sortData->get();
  $data['total_data']=count($sortGetData);
  $data['inactive_data']=count(collect($sortGetData)->whereIn('note_issue_status',0)->toArray());
  $data['active_data']=count(collect($sortGetData)->where('note_issue_status',1)->toArray());
...
public function store(Request $request)
{
  ...
  $data=$request->only('note_issue','note_issue_status');

  if(!empty($request->id))
  {
    $update_data = NoteIssue::valid()->project()->findOrFail($request->id);
    ...
    $save_data = NoteIssue::create($data);
...
{
  $edit_data = NoteIssue::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data = NoteIssue::valid()->project()->findOrFail($id);
  ...
  public function findDepartmentMaxCode(){
    $last_entry_data = NoteIssue::latest()->first();
