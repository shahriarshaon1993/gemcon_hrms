...
class OfficeTimeSetupController extends Controller
{
/**
...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','OfficeTimeSetup')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =OfficeTimeSetup::valid()->project()->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      ...
      $query2->orWhere('office_time_start_date','LIKE','%'.$search_key.'%');
      $query2->orWhere('office_time_end_date','LIKE','%'.$search_key.'%');
    });
    return $query;

  })->where('type',1)->where('project_id',$project_id) ->orderBy($sort,$order);
...
public function store(Request $request)
{
  ...
  $data=$request->only('title','office_start_time','office_end_time','office_time_note','office_time_status','lateConsiderTime','priority','office_type');
...
public function destroy($id)
{
  ...
  public function findPriority(){
    $last_entry_data=OfficeTimeSetup::max('priority');
