...
class ReportController extends Controller
{
  ...
  //    return "Attendance data updated!";
    ...
    public function sbu_wise_employee_find($search_option = null)
    {
        ...
        $emplyId = Employee::valid()->where('employees.employee_status', 1)->where('employees.employee_department', '!=', 132)->where('employee_joining_date', '<=', date('Y-m-d'));
        ...
        if($search_option['sbu_wise_report_type'] == 1){
            ...
            ->select(
                array(
                    ...
                    DB::raw("SUM(CASE
            WHEN employee_type = '1' THEN 1 ELSE 0 END) AS permanent_employee"),
 ...
 if($search_option['sbu_wise_report_type'] == 1){
      ...
      <div class='col-md-12' style='padding: 0px;margin-top: 17px;'>
        ...
        foreach ($employee_info as  $single_data) {
            ...
            // $all_total_employee = ($single_data['permanent_employee'] + $single_data['contractual_employee'] + $single_data['casual_employee'] + $single_data['permanent_employee']);
            $total_employee += $all_total_employee;
            $i++;
            $table .= "<tr>
                    ...
                    <td class='ths' style='padding:2px 10px; text-align: center;vertical-align: middle;'>" . $single_data['permanent_employee'] . "</td>
                    <td class='ths' style='padding:2px 10px; text-align: center;vertical-align: middle;'>" . $single_data['contractual_employee'] . "</td>
                    <td class='ths' style='padding:2px 10px; text-align: center;vertical-align: middle;'>" . $single_data['casual_employee'] + $single_data['temporary_employee'] . "</td>
        ...
        $table .= "</tbody>
          <tbody>
              <tr style='background: #eee;'>
                ...
                <th class='ths' style='padding:2px 10px; width: 20%; text-align: center;vertical-align: middle;background: #dfdfdf;'>" . $total_permanent_employee . "</th>
                ...
                <th class='ths' style='padding:2px 10px; width: 20%; text-align: center;vertical-align: middle;background: #dfdfdf;'>" . $total_casual_employee + $total_temporary_employee . "</th>
    ...
    public function employees_detail_reports($search_option, $checkedattcolsadd)
    {
        ...
        // }
        // echo "<pre>";
        // print_r([$search_option['service_length_from'],$search_option['service_length_to']]);
        // exit();
        if (!empty($search_option['service_length_from']) || !empty($search_option['service_length_to'])) {
            if (!empty($search_option['service_length_from']) && !empty($search_option['service_length_to'])) {
                $all_data1 = collect($all_data1)->where('service_length', '!=', 'No Data!')->where('service_length1', '>=', $search_option['service_length_from'])->where('service_length1', '<=', $search_option['service_length_to'])->toArray();
  ...
  //     }
    ...
    public function find_daily_attendance($report_type, $att_report_type, $employee_sbu, $from_date_formated, $to_date_formated, $checkedattcolsadd, $search_option)
    {
        ...
        //   $date_print['from_date_formated'] = $from_date_formated;
          $emplyId=Employee::whereIn('employee_sbu', $employeeSbu)->where('employees.employee_department', '!=', 132) ->where('employees.employee_status', 1);
          // ->whereIn('employee_department',$employeeDepartment);
          if (!empty($search_option['employee_designation'])) {
          ...
          $allemplyid=array_merge($emplyIds, $resignationsEmpId);
          $attendanceTime = AttendanceSetup::select('attendance_setups.*', 'office_time_setups.office_start_time as office_start_time', 'office_time_setups.title', 'office_time_setups.office_end_time as office_end_time', 'office_time_setups.lateConsiderTime as lateConsiderTime', 'office_time_setups.office_type as office_type', 'office_time_setups.type as type', 'office_time_end_date', 'office_time_start_date')
                          ->leftJoin('office_time_setups', 'office_time_setups.id', '=', 'attendance_setups.attendance_office_time')
        ...
        if (!empty($search_option['OfficeTime'])) {
          ...
          $employee_info=Employee::select('employees.id', 'employees.employee_id_no', 'employees.employee_fullname as employee_full_name', 'employees.employee_sbu', 'employees.employee_section', 'employees.employee_department', 'employees.employee_designation', 'employees.employee_sub_unit', 'employees.employee_sub_unit', 'employees.employee_work_location', 'employees.employee_section', 'employees.employee_sub_section', 'employees.employee_department', 'employees.employee_designation')
            ->valid()
            ->whereIn('employee_sbu', $employeeSbu)
            ->where('employees.employee_department', '!=', 132)
          ...
          $employee_info=$employee_info->orderBy('employees.employee_id_no')->orderBy('employees.employee_sbu')->orderBy('employees.employee_section')->orderBy('employees.employee_department')->get();
        ...
        //   return $indLeaveInfo1;
          ...
          foreach ($employee_info as $key => $value) {
            ...
            $approve_late_request = collect($approve_late_request)->where('late_request_id',$value->id)->first();
            $approve_late_find = array();
            if ($approve_late_request) {
                foreach ($approve_late_request as $date) {
                    array_push($approve_late_find, $date->late_date);
            ...
            //   exit();
              ...
              $manulAbsents=collect($manulAbsent)->where('manual_attendance_date', $from_date_formated)->where('employee_id_no', $value->employee_id_no)->first();
        ...
        //   exit();
          ...
          foreach ($allSubUnits as $key_1 => $value) {
              # code...
              $all_datas=collect($all_data)->where('employee_sub_unit', $value['id'])->sortBy('department_priority')->groupBy('department_id')->toarray();

              foreach ($all_datas as $key_2 => $singleData) {
                  $all_deps=collect($all_data)->where('employee_sub_unit', $value['id'])->where('department_id', $key_2)->sortBy('employee_id_no')->toarray();
                  $department_name=collect($all_data)->where('employee_sub_unit', $value['id'])->where('department_id', $key_2)->first();

                  $table.=" <tr class='body_td'>
                          <th  class='ths' colspan='12' style='width: 5%; text-align: left;vertical-align: middle;'>
