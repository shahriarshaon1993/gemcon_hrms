...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','Section')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =Section::valid()->project()->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('section_name','LIKE','%'.$search_key.'%');
      $query2->orWhere('section_code','LIKE','%'.$search_key.'%');
    });
    return $query;
  ...
  $data['inactive_data']=count(collect($sortGetData)->whereIn('section_status',"!=",1)->toArray());
  $data['active_data']=count(collect($sortGetData)->where('section_status',1)->toArray());
...
public function store(Request $request)
{
  ...
  $data=$request->only('section_name','section_status','priority');

  if(!empty($request->id))
  {
    $update_data=Section::valid()->project()->findOrFail($request->id);
    ...
    $data['section_code'] = $this->findMaxCode();
    ...
    $data['section_status']=1; 
    $save_data=Section::create($data);
...
{
  $edit_data=Section::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=Section::valid()->project()->findOrFail($id);
  ...
  public function findMaxCode(){
    $last_entry_data=Section::max('section_code');
  ...
  public function findPriority(){
    $last_entry_data=Section::max('priority');
  ...
  {
      $data=array();
      $datas=Section::valid()->project()->orderBy('section_name','asc')->get();
      array_push($data,['id'=>'','text'=>'Deselect']);
        foreach ($datas as $value) {
          array_push($data,['id'=>$value['id'],'text'=>$value['section_name'],]);
      }
      $result = $data;
