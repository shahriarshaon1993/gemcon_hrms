...
class SubUnitController extends Controller
{
  ...
  */
  ...
  public function index(Request $request){
    ...
    $permission=collect($cache)->where('menu_uid','=','SubUnit')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
    ...
    $paginate_data =SubUnit::valid()->project()->when($search_key, function($query, $search_key){
      $query->where(function($query2)use($search_key){
        $query2->where('sub_unit_name','LIKE','%'.$search_key.'%');
        $query2->orWhere('sub_unit_code','LIKE','%'.$search_key.'%');
      });
      return $query;
    ...
    $data['inactive_data']=count(collect($sortGetData)->whereIn('sub_unit_status',0)->toArray());
    $data['active_data']=count(collect($sortGetData)->where('sub_unit_status',1)->toArray());
  ...
  public function store(Request $request)
  {
    ...
    $data=$request->only('sub_unit_name','sub_unit_status','priority');

    if(!empty($request->id))
    {
      $update_data=SubUnit::valid()->project()->findOrFail($request->id);
      ...
      $data['sub_unit_code'] = $this->findMaxCode();
      ...
      $data['sub_unit_status']=1; 
      $save_data=SubUnit::create($data);
  ...
  {
    $edit_data=SubUnit::valid()->project()->findOrFail($id);
  ...
  public function destroy($id)
  {
    ...
    $delete_data=SubUnit::valid()->project()->findOrFail($id);
  ...
  public function findMaxCode(){
    $last_entry_data=SubUnit::max('sub_unit_code');
  ...
  public function findPriority(){
    $last_entry_data=SubUnit::max('priority');
  ...
  public function subUnitSelect2()
  {
      $data=array();
      $datas=SubUnit::valid()->project()->orderBy('sub_unit_name','asc')->get();
      array_push($data,['id'=>'','text'=>'Deselect']);
        foreach ($datas as $value) {
          array_push($data,['id'=>$value['id'],'text'=>$value['sub_unit_name'],]);
      }
      $result = $data;
