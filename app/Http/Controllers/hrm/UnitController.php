...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','Unit')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =UnitModel::valid()->project()->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('unit_name','LIKE','%'.$search_key.'%');
    });
    return $query;
  ...
  $data['inactive_data']=count(collect($sortGetData)->whereIn('unit_status',0)->toArray());
  $data['active_data']=count(collect($sortGetData)->where('unit_status',1)->toArray());
...
{
  $edit_data=UnitModel::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  public function findMaxCode(){
    $last_entry_data=UnitModel::latest()->first();
    $department_last_code = isset($last_entry_data['unit_code'])?$last_entry_data['unit_code']:0;
  ...
  public function findPriority(){
    $last_entry_data=UnitModel::max('priority');
  ...
  public function unitSelect2()
  {
      $data=array();
      $datas=UnitModel::valid()->project()->orderBy('unit_name','asc')->get();
      array_push($data,['id'=>'','text'=>'Deselect']);
        foreach ($datas as $value) {
          array_push($data,['id'=>$value['id'],'text'=>$value['unit_name'],]);
      }
      $result = $data;
