...
public function index(Request $request){
  ...
  $paginate_data =WorkArea::valid()->project()->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('work_area_name','LIKE','%'.$search_key.'%');
    });
    return $query;
   ...
   $data['inactive_data']=count(collect($sortGetData)->whereIn('work_area_status',0)->toArray());
   $data['active_data']=count(collect($sortGetData)->where('work_area_status',1)->toArray());
...
public function store(Request $request)
{
  ...
  $validate=[
    'work_area_name'=>'required'
  ];

  $request->validate($validate);
  $data=$request->only('work_area_name','work_area_bangla','work_area_status','priority');

  if(!empty($request->id))
  {
    $update_data=WorkArea::valid()->project()->findOrFail($request->id);
    ...
    $data['work_area_code'] = $this->findMaxCode();
    ...
    $data['work_area_status']=1; 
    $save_data=WorkArea::create($data);
...
{
  $edit_data=WorkArea::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=WorkArea::valid()->project()->findOrFail($id);
  ...
  public function findMaxCode(){
    $last_entry_data=WorkArea::latest()->first();
    $department_last_code = isset($last_entry_data['work_area_code'])?$last_entry_data['work_area_code']:0;
  ...
  public function findPriority(){
    $last_entry_data=WorkArea::max('priority');
  ...
  {
      $data=array();
      $datas=WorkArea::valid()->project()->orderBy('work_area_name','asc')->get();
      array_push($data,['id'=>'','text'=>'Deselect']);
        foreach ($datas as $value) {
          array_push($data,['id'=>$value['id'],'text'=>$value['work_area_name'],]);
      }
      $result = $data;
