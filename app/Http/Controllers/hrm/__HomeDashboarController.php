...
class HomeDashboarController extends Controller
{
    ...
    public function find_dashboard_data(Request $request){
        ...
        // headcount
        $data['dept_headcount'] = (clone $employee_data)->select('departments.department_name', DB::raw('count(departments.id) as employee_no'))->where('employee_department', '!=', '')->groupBy(['employee_department'])->get();
        $data['total_headcount'] = (clone $employee_data)->select('departments.department_name', DB::raw('count(departments.id) as employee_no'))->where('employee_department', '!=', '')->groupBy(['employee_department'])->get()->sum('employee_no');
