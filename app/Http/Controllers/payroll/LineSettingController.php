...
class LineSettingController extends Controller
{
    ...
    $permission=collect($cache)->where('menu_uid','=','LineSetting')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
    ...
    $paginate_data =LineSetting::valid()->project()->when($search_key, function($query, $search_key){
        $query->where(function($query2)use($search_key){
        $query2->where('line_name','LIKE','%'.$search_key.'%');
        });
        return $query;
    ...
    $data['inactive_data']=count(collect($sortGetData)->whereIn('line_status',2)->toArray());
    $data['active_data']=count(collect($sortGetData)->where('line_status',1)->toArray());
    ...
    public function store(Request $request)
    {
        ...
        $data=$request->only('line_name','line_status','priority');
    if(!empty($request->id))
    {
        $update_data=LineSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($request->id);
        ...
        $data['line_code'] = $this->findMaxCode();
        ...
        $data['line_status']=1; 
        $save_data = '';
        $save_data=LineSetting::create($data);
    ...
    public function edit($id)
    {
        ...
        $edit_data=LineSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($id);
    ...
    {
        $delete_data=LineSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($id);
    ...
    public function findMaxCode(){
        $last_entry_data=LineSetting::latest()->first();
        // return response($last_entry_data);
        $last_code = isset($last_entry_data['line_code']) ? $last_entry_data['line_code'] : 0;
        if ($last_code==0) {
            $last_code = 101;
        }else{
