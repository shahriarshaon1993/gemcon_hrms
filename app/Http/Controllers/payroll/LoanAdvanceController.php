...
class LoanAdvanceController extends Controller
{
    public function index(Request $request){
      ...
      $permission=collect($cache)->where('menu_uid','=','LoanAdvance')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
      ...
      $paginate_data =LoanAdvance::valid()->project()
        ...
        return $query;
      })->whereIn('company_sbus.id',$employee_ids['sub'])->whereIn('employees.id',$employee_id)->orderBy($sort,$order);
      ...
      // $employee_data_list=LoanTransaction::valid()->project()->whereIn('employees.id',$employee_id)->groupBy('')->get();
    ...
    public function store(Request $request)
    {
      ...
      if(!empty($employee_reporting_to)){
          if(!empty($request->id))
          {
            $update_data=LoanAdvance::valid()->project()->findOrFail($request->id);
            ...
            $data['loan_status']=1; 
            $save_data=LoanAdvance::create($data);
            ...
            if (!$employee_approvals_data->isEmpty() && !empty($save_data)) {
              ...
              foreach ($employee_approvals_data as $key => $value) {
                ...
                $save_data=DB::table('loan_approvals')->insert($approve_data);
    ...
    public function edit($id)
    {
      ...
      $employee_ids=$employee_list->Employee_id();
      $employee_id=$employee_ids['employee_id'];
      $data=LoanAdvance::valid()->project()->findOrFail($id);
      $employee_data=array();
      ...
      $user_employee_data=Employee::valid()->project()
       ...
       $user_employee_data_all=Employee::valid()->project()
         ...
         ->select(
           ...
           'work_locations.work_location_name',
            'employee_personal_infos.employee_gender'
         )->whereIn('employee_sbu',$employee_ids['sub'])->whereIn('employees.id',$employee_id)
        ...
        $employee_loan=LoanAdvance::valid()->project()
    ...
    {
      $delete_data=LoanAdvance::valid()->project()->findOrFail($id);
      $loanTrangstion=LoanTransaction::valid()->where('loan_adv_id',$id)->get();
      if(!empty($loanTrangstion)){
    ...
    public function findMaxCode(){
      $last_entry_data=LoanAdvance::latest()->first();
    ...
    public function findPriority(){
      $last_entry_data=LoanAdvance::max('priority');
    ...
    public function schedule($id=false){
      ...
      $user_employee_data=Employee::valid()->project()
        ...
        $employee_loan=LoanAdvance::valid()->project()->where('employee_id',$data->employee_id)->where('id',$id)->where('loan_clearance_status', 2)->where('loan_status', 1)->first();

        $paid_amount=  LoanTransaction::valid()->project()
    ...
    public function approveOrReject(Request $request){
        ...
        if (!empty($approval_info)) {
          ...
          $udate_data = DB::table('loan_approvals')->where('loan_apply_id',$loan_id)->where('loan_approve_by',$ea_approve_by)->update($data);
          $udate_data = LoanAdvance::valid()->project()->where('id',$loan_id)->update(array('loan_approve_status'=>$data['loan_approve_status']));
