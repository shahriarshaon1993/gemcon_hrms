...
class MachineSettingController extends Controller
{
    ...
    $permission=collect($cache)->where('menu_uid','=','MachineSetting')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
    ...
    $paginate_data =MachineSetting::valid()->project()->when($search_key, function($query, $search_key){
        $query->where(function($query2)use($search_key){
        $query2->where('machine_name','LIKE','%'.$search_key.'%');
        });
        return $query;
    ...
    $data['inactive_data']=count(collect($sortGetData)->whereIn('machine_status',2)->toArray());
    $data['active_data']=count(collect($sortGetData)->where('machine_status',1)->toArray());
    ...
    public function store(Request $request)
    {
        ...
        $data=$request->only('machine_name','machine_status','priority');
    if(!empty($request->id))
    {
        $update_data=MachineSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($request->id);
        ...
        $data['machine_code'] = $this->findMaxCode();
        ...
        $save_data = '';
        $save_data=MachineSetting::create($data);
    ...
    public function edit($id)
    {
        ...
        $edit_data=MachineSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($id);
    ...
    {
        $delete_data=MachineSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($id);
    ...
    public function findMaxCode(){
        $last_entry_data=MachineSetting::latest()->first();
        // return response($last_entry_data);
        $last_code = isset($last_entry_data['machine_code']) ? $last_entry_data['machine_code'] : 0;
        if ($last_code==0) {
            $last_code = 101;
        }else{
