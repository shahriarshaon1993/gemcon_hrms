...
class ProductSettingController extends Controller
{
    ...
    $permission=collect($cache)->where('menu_uid','=','ProductSetting')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
    ...
    $paginate_data =ProductSetting::valid()->project()->when($search_key, function($query, $search_key){
        $query->where(function($query2)use($search_key){
        $query2->where('product_name','LIKE','%'.$search_key.'%');
        });
        return $query;
    ...
    $data['inactive_data']=count(collect($sortGetData)->whereIn('product_status',2)->toArray());
    $data['active_data']=count(collect($sortGetData)->where('product_status',1)->toArray());
    ...
    public function store(Request $request)
    {
        ...
        $data=$request->only('product_name','product_status','priority');
    if(!empty($request->id))
    {
        $update_data=ProductSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($request->id);
        ...
        $data['product_code'] = $this->findMaxCode();
        ...
        $save_data = '';
        $save_data=ProductSetting::create($data);
    ...
    public function edit($id)
    {
        ...
        $edit_data=ProductSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($id);
    ...
    {
        $delete_data=ProductSetting::valid()->project()->orderBy('priority', 'ASC')->findOrFail($id);
    ...
    public function findMaxCode(){
        $last_entry_data=ProductSetting::latest()->first();
        // return response($last_entry_data);
        $last_code = isset($last_entry_data['product_code']) ? $last_entry_data['product_code'] : 0;
        if ($last_code==0) {
            $last_code = 101;
        }else{
