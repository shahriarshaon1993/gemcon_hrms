...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','ProvidentFund')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data = ProvidentFund::valid()->project()
  ...
  ->selectRaw(
    'provident_funds.*,
     ...
     sum(pf_employee_amount) as pf_employee_amount,
     sum(pf_company_amount) as pf_company_amount,
     sum(pf_profit_interest) as pf_profit_interest
    '
  )
  ->when($search_key, function($query, $search_key){
    $query->where(function($query2)use($search_key){
      $query2->where('provident_funds.pf_employee_amount','LIKE','%'.$search_key.'%')
      ->orWhere('provident_funds.pf_company_amount','LIKE','%'.$search_key.'%')
    ...
    return $query;
  })->whereIn('employees.id',$employee_id)->groupBy('provident_funds.employee_id')->where('provident_funds.project_id',$project_id) ->orderBy($sort,$order);
  // ->paginate($paginate_num);
  $sortData=$paginate_data;
  $sortGetData=$sortData->get();
  $data['total_data']=count($sortGetData);
  $data['active_data']=count(collect($sortGetData)->where('pf_status',1)->toArray());
  $data['inactive_data']=count(collect($sortGetData)->where('pf_status',2)->toArray());
  $data['emp_contribution']=collect($sortGetData)->sum('pf_employee_amount');
  $data['comp_contribution']=collect($sortGetData)->sum('pf_company_amount');
...
public function store(Request $request)
{
  ...
  if(!empty($request->id))
  {
    $update_data=ProvidentFund::valid()->project()->findOrFail($request->id);
    ...
    $data['type']=1; 
    $save_data=ProvidentFund::create($data);
...
{
  // $edit_data=ProvidentFund::valid()->project()->findOrFail($id);
 ...
 // return response($id);
  ...
  $data=ProvidentFund::valid()->project()->where('type',1)->findOrFail($id);
  $employee_data=array();
...
public function destroy($id)
{
  ...
  $delete_data=ProvidentFund::valid()->project()->where('type',2)->findOrFail($id);
  ...
  public function create(){
      ...
      $employee_data=Employee::valid()->whereIn('employees.id',$employee_ids)->project()->get();
  ...
  public function salary_details($employee_id){
    ...
    $emp_salary=ProvidentFund::valid()->project()->where('employee_id', $employee_id)->get();
  ...
  public function provident_fund_details(Request $request){
    ...
    $emp_provident_fund=ProvidentFund::valid()->project()->where('employee_id', $request->page_ref_id)->get();
    $emp_salary=Salary::valid()->project()->where('employee_id', $request->page_ref_id)->get();
    $data['user_employee_data'] = $user_employee_data;
    $data['emp_salary'] = $emp_salary;
    $data['emp_provident_fund'] = $emp_provident_fund;
    return response($data);
  }
