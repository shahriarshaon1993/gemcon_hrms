...
class SalaryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //  ->select('employees.*', 'tamp_salaries.employee_id', 'tamp_salaries.pf_status', 'tamp_salaries.cash_or_bank_status', 'tamp_salaries.salaries','tamp_salaries.bank_account_number', 'tamp_salaries.id as t_id')
        ...
        //               'confirmation_date' => $data->employee_joining_date,
        //               'entry_date' => date('Y-m-d'),
        //               'salary_goes_to' => $data->cash_or_bank_status,
        //               'salary_on_gross_basic' => 1,
        //               'gross_salary' => $data->salaries,
        //               'gross_salary_bangla' => $data->salaries,
        //               'gross_salary_bangla_text' => "No Data",
    ...
    public function store_or_update(Request $request)
    {
        ...
        try {
            ...
            if ($request->bank_gross_salary < 0 || $request->cash_gross_salary < 0 || $request->total_gross_salary < 0) {
                $message = ['status' => 0, 'message' => 'Wrong entry! Negative salary not acceptable.'];
                return response($message);
            }
            if($request['one_off_bonus'] >= 0){
                ...
                EmployeeHistory::updateOrCreate(
            ...
            if ($request->bank_gross_salary >= 0) {
                ...
                if ($request->bank_id) {
                    ...
                    $data_bank['car_allowance_amount'] = 0;
                    $data_bank['others_allowance'] = 0;
                    $data_bank['phone_allowance'] = 0;
                    ...
                    $update_data = Salary::valid()->findOrFail($request->bank_id);
            ...
            if ($request->cash_gross_salary >=0) {
                ...
                if ($request->cash_id) {
                    ...
                    $data_cash['others_allowance'] = $request->others_allowance ?? 0;
                    $data_cash['phone_allowance'] = $request->phone_allowance ?? 0;
                    ...
                    $data_cash['car_allowance_amount'] = $car_allowance_amount;
                    ...
                    $data_cash['car_allowance_status'] =$request->car_allowance_status ?? 0;
                    $data_cash['car_allowance_amount'] = $car_allowance_amount ?? 0;
                    $data_cash['others_allowance'] = $request->others_allowance ?? 0;
                    $data_cash['phone_allowance'] = $request->phone_allowance ?? 0;
    ...
    public function old_store_or_update(Request $request)
    {
    ...
    public function user_update_salary($employee_id)
    {
        ...
        ->selectRaw(
            ...
            sum(phone_allowance) as mobile_allowance
            '
        )
        ...
        $data['car_allowance_amount'] = $data['cash_car_allowance_amount'] ?? 0;
        $data['others_allowance'] = $data['cash_others_allowance'] ?? 0;
        $data['phone_allowance'] = $data['cash_phone_allowance'] ?? 0;

        $all_gross_salary = collect($emp_salary)->sum('gross_salary');
        $data['total_gross_salary'] = ($all_gross_salary ?? 0) + ($data['cash_car_allowance_amount'] ?? 0) + ($data['cash_others_allowance'] ?? 0) +  ($data['cash_phone_allowance'] ?? 0); 
        ...
        if($data['car_allowance_status'] == 0 || $data['car_allowance_status'] == 2){
            $data['car_allowance_status'] = 2;
        }
        return response($data);
    }
