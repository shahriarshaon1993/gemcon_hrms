...
use App\Model\payroll\TimeNProductionModel;
...
class TimeNProductionRateController extends Controller
{
/**
...
public function index(Request $request){
  ...
  $permission=collect($cache)->where('menu_uid','=','TimeNProductionRate')->where('role_id',Auth::guard('user')->user()->role_id)->toArray();
  ...
  $paginate_data =TimeNProductionModel::valid()->project()
    ->leftJoin('employees', 'employees.id', '=', 'time_production_rates.employee_id')
    ->leftJoin('designations', 'designations.id', '=', 'employees.employee_designation')
    ->select('time_production_rates.*','employees.employee_fullname','designations.designation_name','employees.employee_id_no')
    ->when($search_key, function($query, $search_key){
      $query->where(function($query2)use($search_key){
        $query2->where('time_production_rates.attendance_type','LIKE','%'.$search_key.'%');
        $query2->orWhere('employees.employee_id_no','LIKE','%'.$search_key.'%');
      });
      return $query;

    })->where('time_production_rates.project_id',$project_id)->whereIn('time_production_rates.employee_id',$employee_id)->orderBy($sort,$order);
    $sortData=$paginate_data;
    $sortGetData=$sortData->get();
    $data['total_data']=count($sortGetData);
    $data['inactive_data']=count(collect($sortGetData)->whereIn('rate_status',2)->toArray());
    $data['active_data']=count(collect($sortGetData)->where('rate_status',1)->toArray());
...
public function store(Request $request)
{
  ...
  $data=$request->only('employee_id','rate_type','rate_amount','time_production_qty','rate_status');

  if(!empty($request->id))
  {
    $update_data=TimeNProductionModel::valid()->project()->findOrFail($request->id);
    ...
    $data['rate_status']=1; 
    $save_data=TimeNProductionModel::create($data);
...
public function edit($id)
{
   ...
   $employee_ids=$employee_list->Employee_id();
   $employee_id=$employee_ids['employee_id'];
  $data=TimeNProductionModel::valid()->project()->findOrFail($id);
...
public function destroy($id)
{
  ...
  $delete_data=TimeNProductionModel::valid()->project()->findOrFail($id);
