            ...
            <div class="toolbar no-padding ">
                ...
                <modal name="myModal" width="660" height="auto" :clickToClose="false">
                    <div v-if="modal_loading">
                        <div class="widget-header">
                            <h4><i class="icon-reorder"></i>Module Forms</h4>
                        ...
                        <div class="modify-wraper">
                            <form @submit.prevent="add({add:'module_data/add'})" class="form-horizontal  row-border" id="validate-1">
                                <div class="my-form-wraper">
                                    ...
                                    <div class="form-group modify-input">
                                        <label class="col-md-3 control-label">Module Name<span class="required">*</span></label>
                                        <div class="col-md-9" :class="{ 'has-error': $v.form_data.name.$error }">
                                            {{$v.form_data.name.$touch()}}
                                            <input v-model="form_data.module_name" type="text" class="form-control required">
                                        </div>
                                    </div>
                                    <div class="form-group modify-input">
                                        <label class="col-md-3 control-label">Image<span class="required">*</span></label>
                                        <div class="file-upload-form" style="padding:8px 0px 0px 59px;">
                                            <input type="file" v-on:change="onFileChange" accept="image/*"> 
                                        </div>
                                        <div class="col-md-12">
                                            <div class="image-preview"  style="width: 243px; height: 50px; padding: 8px 36px 0px 36px;" >
                                                <img :src="form_data.icon" style="width: 46px;" class="preview">
                                            </div>
                                        </div>
                                    </div>
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <tbody v-if="Object.keys(paginate_data.data).length > 0">
                        <tr v-for="(form_data, index) in paginate_data.data" v-bind:key="form_data.id">
                            <td style="vertical-align: inherit;">{{order_no + index + 1}}</td>
                            <td style="vertical-align: inherit;">{{form_data.module_name}}</td>
                             <td style="vertical-align: inherit;">
                                <img class="img-design img-responsive" :src="`assets/icon/${form_data.icon}`"> 
                            </td>
                            <!-- <td>{{form_data.account_number}}</td> -->
                            <!-- <td> <samp v-if="form_data.status==1" > Active </samp>   <samp v-else> Inactive </samp> </td> -->
                            <td style="width: 20%;vertical-align: inherit;" class="action">
                                <button @click="getModalData($event,{dataUrl:'module_data/edit/'+form_data.id})" class="btn-xs btn-info" data-toggle="modal" data-target="#myModal"><i class="icon-edit"></i> Edit</button>
                                <button @click="deleteItem({delUrl:'module_data/delete/'+form_data.id})" class="btn-xs btn-danger"><i class="icon-trash"></i> Delete</button>
                            </td>
                        </tr>   
                    </tbody>
                    <tbody v-else>
    ...
    export default {
        ...
        methods: {
          ...
          createImage(file) {
              ...
              let vm = this;
              reader.onload = (e) => {
                  this.form_data.image = e.target.result;
              };
