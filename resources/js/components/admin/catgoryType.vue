...
<div>
    <div v-if="page_loading" class="widget box">
        <div class="widget-header">
            <h4><i class="icon-reorder"></i>Catgory Type</h4>

            <div class="toolbar no-padding ">
                <div @click="getModalData($event),form_data.status=1" class="btn-group"> <span class="btn btn-xs btn-info"><i class="icon-plus"></i>Add New</span></div>
                ...
                <modal name="myModal" width="677" height="auto" :clickToClose="false">
                    <div v-if="modal_loading">
                        <div class="widget-header">
                            <h4><i class="icon-reorder"></i> Catgory Type Form</h4>
                        ...
                        <div class="modify-wraper">
                            <form @submit.prevent="add({add:'add/CatgoryType'})" class="form-horizontal  row-border" id="validate-1">
                                <div class="pos-form-wraper">
                                    ...
                                    <div class="form-group modify-input">
                                        <label class="col-md-3 control-label">Catgory Type Name</label>
                                        <div class="col-md-7">
                                            <input v-model="form_data.catgory_name" type="text" class="form-control multiselect__tags required">
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-6">
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <tbody v-if="Object.keys(paginate_data.data).length > 0">
                        ...
                        <tr v-for="(form_data, index) in paginate_data.data" v-bind:key="form_data.id">
                            ...
                            <td class="action">
                                <button @click="getModalData($event,{dataUrl:'edit/CatgoryType/'+form_data.id})" class="btn-xs btn-info" data-toggle="modal" data-target="#myModal"><i class="icon-edit"></i> Edit</button>
                                <button @click="deleteItem({delUrl:'delete/CatgoryType/'+form_data.id})" class="btn-xs btn-danger"><i class="icon-trash"></i> Delete</button>
                            </td>
                        </tr>
                    </tbody>
                    <tbody v-else>
