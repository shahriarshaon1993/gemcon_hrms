...
<div>
    <div v-if="page_loading" class="widget box">
        <div class="widget-header">
            <h4><i class="icon-reorder"></i>Employee Department</h4>

            <div class="toolbar no-padding ">
                <div @click="getModalData($event,{dataUrl:'create/em_department'})" class="btn-group"> <span class="btn btn-xs btn-info"><i class="icon-plus"></i>Add New</span></div>
                ...
                <modal name="myModal" width="677" height="auto" :clickToClose="false">
                    <div v-if="modal_loading">
                        <div class="widget-header">
                            <h4><i class="icon-reorder"></i> Employee Department Form</h4>
                        ...
                        <div class="modify-wraper">
                            <form @submit.prevent="add({add:'add/em_department'})" class="form-horizontal  row-border" id="validate-1">
                                <div class="pos-form-wraper">
                                    <div v-if="errors" class="alert alert-danger">
                                     ...
                                     <div class="form-group modify-input">
                                        ...
                                        <div class="col-md-6">
                                            <input v-model="form_data.department_code" readonly type="text" class="form-control required">
                                        </div>
                                    </div> 
                                    <div class="form-group modify-input">
                                        <label class="col-md-3 control-label">Department Name</label>
                                        <div class="col-md-6">
                                            <input v-model="form_data.department_name" type="text" class="form-control required">
                                        </div>
                                    </div> 
                                    <div class="row">
                                        <div class="col-md-6">
                ...
                <table class="table table-striped table-bordered table-hover">
                    <thead>
                        <tr>
                            ...
                            <td class="sortable" v-bind:class="getSortingClass('department_code')" @click="sortingChanged('department_code')">
                            Department Code
                            </td>    
                    ...
                    <tbody v-if="Object.keys(paginate_data.data).length > 0">
                        ...
                        <tr v-for="(form_data, index) in paginate_data.data" v-bind:key="form_data.id">
                            <td>{{index + 1}}</td>
                            <td>  {{form_data.department_code }}</td>
                            <td>  {{form_data.department_name }}</td>
                            <td class="action">
                                <button @click="getModalData($event,{dataUrl:'edit/em_department/'+form_data.id})" class="btn-xs btn-info" data-toggle="modal" data-target="#myModal"><i class="icon-edit"></i> Edit</button>
                                <button @click="deleteItem({delUrl:'delete/em_department/'+form_data.id})" class="btn-xs btn-danger"><i class="icon-trash"></i> Delete</button>
                            </td>
                        </tr>
                    </tbody>
                    <tbody v-else>
