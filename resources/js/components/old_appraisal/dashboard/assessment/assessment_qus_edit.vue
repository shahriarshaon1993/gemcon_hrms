...
export default {
  ...
  methods: {
    async getItems() {
      ...
      try {
        await axios
          .get(this.api_url + "questions/" + this.id, {
            headers: {
          ...
          .then(({ data }) => {
            if (data.success) {
              this.item = data.data;
              this.addForm.vQuestion = data.data.vQuestion;
              this.qus.children.name = data.data.answersjoin.vAnswer;
              this.qus.children.mark = data.data.answersjoin.mark;
              this.qus.children = [];
              for (const [key, value] of Object.entries(
                data.data.answersjoin
              )) {
                
                console.log(key, value);
                this.qus.children.push({ name: value.vAnswer, mark: value.mark });
              }
              console.log("this.qus", this.qus);
            }
            loader.hide();
          });
      } catch (error) {
    ...
    create() {
      ...
      try {
        ...
        this.addForm
          .put(this.api_url + "questions/" + this.id, {
            headers: {
              "Content-Type": "application/json",
              //   Authorization: this.token ? `Bearer ${this.token}` : "",
            },
