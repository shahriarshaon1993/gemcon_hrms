                    ...
                    <div class="card-header">
                       <div class="row">
                           <div class="col-12 col-sm-6 col-md-12" style="padding: 5px 10px;">
                               <h3 class="card-title d-none d-md-block">Payroll Permission Assign</h3>
                               <span class="float-sm-right" style="float: right;">
                                <!-- v-if="lists.add=='add'" -->
                                 <div   @click="getModalData($event,{dataUrl:'create/payroll_permission_assign'})" class="btn-group"> <span class="btn btn-sm btn-info"><i class="icon-plus"></i>Add New</span></div>
                        ...
                        <thead>
                         ...
                         <tbody  v-if="Object.keys(paginate_data.data).length > 0">
                          <tr v-for="(form_data, index) in paginate_data.data" v-bind:key="form_data.id" i = index>
                            ...
                            <td class="text-left">{{form_data.employee_id_no}}</td>
                            <td class="text-left">{{form_data.employee_fullname}}</td>
                            <td class="text-left">{{form_data.sbu_name}}</td>
                            <td class="text-center">{{form_data.permission_group}}</td>
                            ...
                            <td class="text-center">
                              ...
                              <button   class="btn btn-xs btn-info" @click="getModalData($event,{dataUrl:'edit/payroll_permission_assign/'+form_data.id},setModalData)" title="Edit" > <i class="fa fa-edit"> </i> Edit </button>
                            <!-- v-if="lists.delete=='delete'" -->
                              <button  class="btn btn-xs btn-danger"  @click="deleteItem({delUrl:'delete/payroll_permission_assign/'+form_data.id})" title="Delete" ><i class="fa fa-trash"></i> Delete</button>
                            </td>
                          </tr>
                        </tbody>
                         <tbody v-else>
          ...
          <modal class="" width= "40%" name="myModal" height="auto" :clickToClose="false">
               <div v-if="modal_loading">
                   <div class="widget-header modal-header">
                       <h4><i class="fa fa-bars"></i> Permission Assign</h4>
                   ...
                   <div class="modify-wraper">
                         <form @submit.prevent="add({add:'add/payroll_permission_assign'},resetModal)" class="form-horizontal  row-border" id="validate-1" style="padding-top:30px; padding-bottom: 30px;">
                           <div class="">
                            <br>
                             <div class="col-md-12">
                                <div class="row">
                                  <div class="col-md-12">
                                    <div class="form-group">
                                       <label class="col-md-12 control-label">Employee Name</label>
                                       <div class="col-md-12 inputGroupContainer">
                                          <div class="input-group">
                                           <span class="input-group-addon"><i class="glyphicon glyphicon-home"></i></span>
                                           <vue-select v-model="form_data.sbu_name_value" :options="option_data.employee_data" @select="onSelectEmployee" placeholder="Select one" label="text" track-by="text"></vue-select>
                                         </div>
                                       </div>
                                    </div>
                                  </div>
                                  <div class="col-md-12">
                                    <div class="form-group">
                                       <label class="col-md-12 control-label">Permission Group</label>
                                       <div class="col-md-12 inputGroupContainer">
                                          <div class="input-group">
                                           <span class="input-group-addon"><i class="glyphicon glyphicon-home"></i></span>
                                           <vue-select v-model="form_data.Permission_groups" :options="option_data.payrollPermission" @select="onSelectpayrollPermission" placeholder="Select one" label="text" track-by="text"></vue-select>
                                         </div>
                                       </div>
                                    </div>
                                  </div>
    ...
    export default {
        ...
        methods: {
          onSelectEmployee(option){
            this.form_data.company_sbu_id= option.sbu_id;
            this.form_data.employee_id= option.id;
          },
          onSelectpayrollPermission(option){
            console.log(option);
            this.form_data.assign_id= option.id;
            this.form_data.permission_grade_start= option.grade_start;
            this.form_data.permission_grade_end= option.grade_end;
          },
          // setModalData(){
          //   this.sbu_name_value=this.form_data.sbu_name_value;
          //   this.Permission_groups=this.form_data.jobgrade_name_value_start;
