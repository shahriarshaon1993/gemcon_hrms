...
<div>
    <div v-if="page_loading" class="widget box">
                    ...
                    <div class="card-header">
                       <div class="row">
                           <div class="col-12 col-sm-6 col-md-12" style="padding: 5px 10px;">
                               ...
                               Weekly Payroll Process List</h3>
                               <span class="float-sm-right" style="float: right;">
                                 <!-- <div v-if="lists.add=='add'"  @click="getModalData($event,{dataUrl:'create/weekly_payroll_list'},resetModal)" class="btn-group"> <span class="btn btn-sm btn-info"><i class="fa fa-plus"></i> Add Process</span></div> -->
                  ...
                  <table id="employeeTable" class="table table-bordered table-striped employeeTable">
                    <thead>
                      <tr>
                        ...
                        <th class="text-center">Payroll Month</th>
                     ...
                     <tbody  v-if="Object.keys(paginate_data.data).length > 0">
                      <tr v-for="(form_data, index) in paginate_data.data" v-bind:key="form_data.id" i=index>
                        ...
                        <td class="text-center">{{form_data.paymonth}}</td>
                        ...
                        <td class="text-center">{{form_data.startdate}}</td>
                        <td class="text-center">{{form_data.enddate}}</td>
                        <td class="text-center">
                          <span v-if="form_data.remarks==1">{{ 'Time Based' }}</span>
                          <span v-if="form_data.remarks==2">{{ 'Production Based' }}</span>
                          <span v-if="form_data.remarks==3">{{ 'Residential Based' }}</span>
                          <span v-if="form_data.remarks==4">{{ 'TR' }}</span>
                        </td>
                        <td class="text-center">-</td>
                        <td class="text-center">{{form_data.total_salary_amount}}</td>
                        <td class="text-center">
                          <span v-if="form_data.status==1" style="color:green;">Active</span>
                        ...
                        <td class="text-center" style="width: 25%;">
                          ...
                          <router-link href="#" :to="'/weekly_payroll_list_details/'+form_data.id" class="btn btn-xs btn-success" title="Add More Data">
                            <i class="fa fa-search" aria-hidden="true"></i> Details
                          </router-link>
                          
                          <button v-if="form_data.settlement==1" class="btn btn-xs btn-info" @click="getModalData($event,{dataUrl:'final_process/weekly_payroll_list/'+form_data.id})" title="Edit" > <i class="fa fa-tasks"> </i> Final Process </button>
                          <button v-if="form_data.settlement==2" class="btn btn-xs btn-info" title="Final Process Completed!" @click="AccessDenied($event,value=' Final Process Completed!')" style="opacity: 0.5"> <i class="fa fa-tasks"> </i> Final Process </button>
                                                    
                           <button  v-if="form_data.settlement==1" class="btn btn-xs btn-danger"  @click="deleteItem({delUrl:'delete/weekly_payroll_list/'+form_data.id})" title="Delete" ><i class="fa fa-trash"></i> Delete</button>
                           <button  v-if="form_data.settlement==2" class="btn btn-xs btn-danger" title="" @click="AccessDenied($event,value=' Final Process Completed!')" style="opacity: 0.5"><i class="fa fa-trash"></i> Delete</button>
                        </td>
                      </tr>
                    </tbody>
                     <tbody v-else>
       ...
       <modal class="" name="myModal" height="auto" :clickToClose="false" width="400">
            <div v-if="modal_loading">
                <div class="widget-header modal-header">
                    <h4><i class="fa fa-info"></i> Are you sure? </h4>
                ...
                <div class="modify-wraper modal-body">
                  <div class="container">
                    <form @submit.prevent="add({add:'final_process_submit/payroll_list'},resetModal)" class="form-horizontal  row-border" id="validate-1">
                           ...
                           <div class="col-md-12 inputGroupContainer">
                              <div class="input-group" style="padding:30px;">
                               ...
                               <input type="hidden" v-model="form_data.id">
                                ...
                                <button type="button" @click="hideModal" class="btn btn-sm btn-default float-right col-md-2 offset-md-4" style="margin-right: 20px;">No</button>
                                <input type="submit"   tabindex="4" value="Yes" class="btn btn-sm btn-success float-right col-md-2">
                              </div>
                           </div>
                        </div>
                      </form>
  ...
  export default {
    ...
    methods:{
      ...
      getModalDataOther(id){
        // console.log('aaaaaa');
        let uri = URL.baseUrl('other_create/weekly_payroll_list/'+id);
        console.log(uri);
        axios.get(uri)
        .then(res => {
